...
public class TestProduct {
	...
	public static void main(String[] args) {
		...
		//
		List<String> prodNames = prodList.stream().filter(kev -> kev.deliveryCharges == 0.0f).map(hasan -> hasan.name)
				.collect(Collectors.toList());
		System.out.println(prodNames);

		Set<String> setNames = prodList.stream().filter(kev -> kev.deliveryCharges == 0.0f).map(hasan -> hasan.name)
		...
		prodList.stream().map(j -> j.name).forEach(m -> System.out.println(m));
		System.out.println("Sorted product list");
		prodList.stream().map(p -> p.name).sorted().forEach(f -> System.out.println(f));
